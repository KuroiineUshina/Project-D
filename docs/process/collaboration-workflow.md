...
Project D는 여러 컴퓨터, 여러 에이전트, 여러 세션에서 이어서 작업할 수 있도록 GitHub 저장소를 단일 기준점으로 사용한다.
...
다른 에이전트는 `main`에 직접 큰 변경을 쌓지 말고, 작업 결과를 문서와 커밋으로 남긴다.
...
- 평가만 요청받은 에이전트는 파일 수정과 커밋을 하지 않는다.
